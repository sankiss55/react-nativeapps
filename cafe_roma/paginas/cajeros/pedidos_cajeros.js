      ...
      try {
        ...
        const unsubscribe = onSnapshot(q, (querySnapshot) => {
          ...
          const pedidosAgrupados = pedidosUsuarios.reduce((acc, pedido) => {
            const { id_usuario } = pedido;

            if (!acc[id_usuario]) {
              acc[id_usuario] = [];
            }
            acc[id_usuario].push(pedido);

            return acc;
          }, {});

          // Filtrar usuarios cuyos pedidos no están completamente atendidos
          const usuariosConPedidosAtendidos = Object.entries(pedidosAgrupados).reduce((acc, [id_usuario, pedidos]) => {
            const todosAtendidos = pedidos.every((pedido) => pedido.atendido);
            if (todosAtendidos) {
              acc[id_usuario] = pedidos;
            }
            return acc;
          }, {});

          setMesas(usuariosConPedidosAtendidos);
        });
    ...
    <View style={styles.container}>
      <View style={styles.container_top}>
        ...
        >
          ...
          <Svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="http://www.w3.org/2000/svg" width="24px" fill="black">
      ...
      <ScrollView>
        <Grid>
          {Object.keys(mesas).map((id_usuario, index) => (
            <Row key={id_usuario} style={styles.row_mesas}>
              <Pedidos_mesas
                onPress={() =>
                  navigation.navigate("PedidosTerminados", {
                    pedidos: mesas[id_usuario],
                    id_usuario_pedido: id_usuario,
                  })
                }
                title={`Pedido para: ${mesas[id_usuario][0].mesa=='parallevar'?'Para llevar':mesas[id_usuario][0].mesa}`}
                text={`Pedidos: ${mesas[id_usuario].length}`}
                icon={
                  <Svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="http://www.w3.org/2000/svg" width="24px" fill="#000000">
